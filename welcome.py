...
class WelcomeMod(loader.Module):
	...
	async def welcomecmd(self, message):
		...
		if args == "clearall":
			...
			return await message.edit("<b>[Welcome Mode]</b> Все настройки "
			                          "модуля сброшены.")
	...
	async def setwelcomecmd(self, message):
		...
		try:
			if not args and not reply:
				return await message.edit(f'<b>Приветствие новых '
				                          f'пользователей в '
				                          f'"{chat.title}":</b>\n\n'
				...
				self.db.set("Welcome", "welcome", welcome)
				return await message.edit("<b>Новое приветствие установлено "
				                          "успешно!</b>")
		except KeyError:
