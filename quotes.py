...
class mQuotesMod(loader.Module):
		...
		for msg in messages_client:
			...
			if msg.fwd_from:
				if msg.fwd_from.saved_from_peer:
					...
					id = msg.fwd_from.from_id.channel_id
				elif isinstance(msg.fwd_from.from_id, PeerUser):
					id = msg.fwd_from.from_id.user_id
				else:
					id = None
				if not id:
				...
				if not message.chat or \
					...
					if not pfp:
						pfp = await self.client \
							.download_profile_photo(msg.sender_id,
													"mishase_cache/")
						if pfp:
							media_files[
								"@av{}".format(str(id).lstrip("-"))
	...
	async def fquotecmd(self, message):
		...
		if avatar:
			...
			files.append(("files", (f'@av{str(user.id if not str(user).isdigit() else user).lstrip("-")}', open(avatar, "rb"), "image/jpg")))
		else:
