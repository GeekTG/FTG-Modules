		...
		if message.is_reply:
			...
			if isinstance(data, bool):
				await utils.answer(message,
				                   "<code>Reply to pic or stick!</code>")
				return
		else:
		...
		async with message.client.conversation(chat) as conv:
			...
			try:
				...
				await (await conv.wait_event(
					events.NewMessage(incoming=True, from_users=chat))).delete()
				await x.delete()
				...
				for im in images:
					...
					blank.name = name + ".png"
					blank.seek(0)
					x = await message.client.send_file(chat, blank,
					                                   force_document=True)
					await (await conv.wait_event(
						events.NewMessage(incoming=True,
						                  from_users=chat))).delete()
					await x.delete()
				...
				await (await conv.wait_event(
					events.NewMessage(incoming=True, from_users=chat))).delete()
				await x.delete()
				x = await message.client.send_file(chat, fface,
				                                   force_document=True)
				await (await conv.wait_event(
					events.NewMessage(incoming=True, from_users=chat))).delete()
				await x.delete()
...
async def dotify(message, reply, pix, mode):
	...
	w, h = im.size
	img = Image.new(im_.mode,
	                (w * count + (count // 2), h * count + (count // 2)), 0)
...
async def KZD(message, type):
	...
	E = Image.open(R)
	B, C = E.size
	if B % 2 != 0 and type in [1, 2] or C % 2 != 0 and type in [3,
	                                                            4]: E = E.resize(
		(B + 1, C + 1));C, B = E.size
