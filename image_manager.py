...
class ImageManagerMod(loader.Module):
	...
	async def delbgcmd(self, m):
		...
		if not im:
			return await utils.answer(m, s + "Reply to image")
	...
	async def resizecmd(self, m):
		...
		if not args:
			return await utils.answer(m, "Specify x:int y:int")
	...
	async def rotatecmd(self, m):
		...
		args = utils.get_args_raw(m)
		if not args:
			return await utils.answer(m, "Specify <rotate:int>")
		rr = re.compile(r"^(-*\d+)$")
		if not rr.match(args):
			return await utils.answer(m, "Specify <rotate:int>")
	...
	async def contrstcmd(self, m):
		...
		args = utils.get_args_raw(m)
		if not args:
			return await utils.answer(m, "Specify level:float or int")
