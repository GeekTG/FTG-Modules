...
class ConthelperMod(loader.Module):
    ...
    async def addcontcmd(self, message):
        ...
        if message.chat_id != (await message.client.get_me()).id and message.is_private:
            ...
            if not user:
                ...
                return
        await message.client(functions.contacts.AddContactRequest(id=user.id, first_name =" ".join(args[0:]), last_name=' ', phone='' , add_phone_privacy_exception=False))
    ...
    async def renamecmd(self, message):
        ...
        except: return await message.edit("<b>Something went wrong (come up with different reasons).</b>")  
