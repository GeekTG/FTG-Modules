...
class TextGeneratorMod(loader.Module):
    ...
    async def gptcmd(self, message):
        ...
        message = await utils.answer(message, self.strings('wait', message))
        response = (await utils.run_sync(requests.post, "https://api.aicloud.sbercloud.ru/public/v1/public_inference/gpt3/predict",
                                                         json={"text": text})).json()
        return await utils.answer(message,'<strong>' + text + '</strong>' + response["predictions"].split(text.split()[-1], maxsplit=1)[1])
